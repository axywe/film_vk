...
func TestUpdateMovie(t *testing.T) { // TODO: Add films and users
	db := testutils.SetupDB(t)
	defer db.Close()
...
func TestDeleteMovieEnsuringDeletion(t *testing.T) {
	...
	if createRr.Code != http.StatusCreated {
		t.Fatalf("Failed to create movie for deletion test, status code: %v", createRr.Code)
	}
	...
	deleteReq, err := http.NewRequest(http.MethodDelete, fmt.Sprintf("/movies?id=%d", createdMovie.ID), nil)
	if err != nil {
	...
	var movieCount int
	err = db.QueryRow("SELECT COUNT(*) FROM movies WHERE id = $1", createdMovie.ID).Scan(&movieCount)
